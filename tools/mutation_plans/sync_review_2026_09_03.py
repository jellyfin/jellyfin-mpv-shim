...
MUTATIONS = [
    ...
    ('playlist: an emptied membership leaves its record behind',
     'jellyfin_mpv_shim/sync/db.py',
     '                if not written:\n                    self._conn.execute(\n                        "DELETE FROM playlists WHERE playlist_id=?" + clause,',
     '                if False:\n                    self._conn.execute(\n                        "DELETE FROM playlists WHERE playlist_id=?" + clause,'),
    ('playlist: an ownership read answers without the row',
     'jellyfin_mpv_shim/sync/db.py',
     ...
     '            "SELECT pi.item_id FROM playlist_items pi "\n            "WHERE pi.playlist_id=? AND pi.owned=1" + clause,'),
    ('playlist: the ownership map answers without the row',
     'jellyfin_mpv_shim/sync/db.py',
    ...
    ('enqueue: uncancel every item, including the declined ones',
     ...
     '        for item in items:\n            iid = item.get("Id")\n'
     '            self._uncancel(iid)\n'
     '            verdict = self.claim_identity(iid, item, content_id,'),
