...
- `mirror` (`display_mirror.py`) — optional Chromecast-like preview screen (idle "Ready to cast" backdrop + `DisplayContent` item preview), only loaded if `display_mirroring` is enabled and `Pillow` is installed. Rendered with **mpvtk inside the player's mpv window** (backdrop + gradient + text baked into one full-window bitmap); `mirror.run()` attaches to `playerManager`'s mpv and becomes the main loop. Mutually exclusive with the mpvtk browser (both own the mpv window — mirroring wins). Public surface preserved: `run`/`stop`/`display_content`/`get_webview`→`hide`/`show`.
- **Library browser UI** (`browser_ui` setting): `"mpvtk"` (default) renders the browser *inside the player's mpv window* via `mpvtk_browser/` (in the main process, attached to `playerManager`'s mpv); `"tk"` runs the legacy Tkinter browser in a `multiprocessing` child (`gui_mgr.BrowserProcess`). The Tk browser is slated for removal once mpvtk parity is signed off — see `mpvtk/MIGRATION.md`.
