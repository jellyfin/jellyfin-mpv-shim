...
practice; extracting it makes that a fact about the module graph rather than
a convention, which is what lets the eventual page objects be constructed
(and tested) without dragging ``player.py`` in. See
``docs/ARCHITECTURE_TARGET.md`` §1.2.
...
log = logging.getLogger("mpvtk_browser.player_gateway")
...
class PlayerGateway:
    ...
    def any_client(self):
        ...
        import random
        try:
            clients = list(clientManager.clients.values())
        except Exception:
            return None
    ...
    def client_for(self, server_uuid):
        ...
        try:
            return clientManager.clients.get(server_uuid)
        except Exception:
