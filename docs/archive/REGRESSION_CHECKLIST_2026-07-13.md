...
- [X] Another member stops the group → this client stops too, and is **still
      in the group** afterwards (a later play from another member reaches it).
- [X] Throttle the network mid-playback so mpv stalls on its cache → the group
      pauses for this client instead of leaving it behind and yanking it.
- [X] Local pause, then local unpause → both reach the group. The unpause used
