...
| 10 | **Make `notify_updates` tri-state** (`default` / `enabled` / `disabled`), resolve `default` by platform — off inside a Flathub Flatpak, on elsewhere — and add a `CONFIG_VERSION` 6 migration — **DONE**, with the SteamOS gamepad step in the same migration and the probe in `hostinfo.py` | §5.3 | **Low-moderate.** The risk is the migration, not the setting: conf.json persists every key, so a stored `true` is indistinguishable between "never touched" and "explicitly wanted", and migrating it to `default` silently loses the second. Migrate a stored `false` to `disabled` so a deliberate opt-out is never stomped. |
| 10a | **Retarget the notice's wording inside Flatpak** — say `flatpak update` — for the users who switch it back on, and for hand-delivered builds marked `JMS_UPDATE_CHECK=1` — **DONE**, banner and OSD toast both. **The [Open] link is kept**, correcting this row: [iw], once row 10 made the notice off-by-default in a Flatpak, anyone seeing it there asked for it and may want the release notes, which is what the page is good for even where it cannot deliver the build | §5.3 | **Low.** Independent of 10; the banner still exists wherever it is enabled, and [Open] on a Flathub install has never been the right action. |
| 10b | **A persisted per-version skip**, beside the existing session-scoped [Dismiss] — **DONE**: the banner is now [Details…] [Later] [Ignore], and `update_skip_version` is matched by equality | §5.3; `has_notified` is in-memory and `_dismiss_update` only repaints — CONFIRMED | **Low.** One `str` key, `INTERNAL` in `test_docs_coverage.py`, equality not ordering. Matters most on Windows and pip, where the notice stays on by default and the every-launch nag is what drives people to disable it. |
...
| Flatpak, unmarked (i.e. Flathub) | *"Update available: X — run `flatpak update`"* | **[Open]** → releases page, **kept** — see the correction at row 10a: the notice is off by default here now, so anyone seeing it asked for it and the release notes are the thing the page is still good for |
...
#### An interface language selector — DONE, with the detection fix under it
