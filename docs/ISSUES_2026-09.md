...
auto-hide until the pointer moves) — **the verdict recorded here was wrong and
is struck.**

~~is the #700 hover strand, fixed by `2035d720`, which landed **after** pre14
was cut. The reporter confirms it is fixed on dev. Nothing to do; it ships in
pre15.~~

`git merge-base --is-ancestor 2035d720 v3.0.0pre14` returns **true**:
`2035d720` is 2026-08-24 and `v3.0.0pre14` is 2026-08-28, so the reporter was
already running the claimed fix and "nothing to do" had no basis. Worse, the
whole #700 synth-hover machinery landed inside the pre13→pre14 window, which is
the regression window #724 names — making it a *candidate cause* rather than the
cure.

The mechanism is now traced to site A4 (`renderer.lua:6020-6023`), which is
still open at HEAD. But the reporter states issue 2 is fixed on current dev
builds, and nothing in the seven renderer commits between pre14 and 3.0.0
touches that site, so **why it stopped reproducing for them is unresolved** —
ask before claiming it closed. See `docs/RELEASE_SHAPE_POST_3.0.0.md` §1.2-1.3.
...
**STRUCK — do not act on this. The branch is not dead, and deleting it would
remove the sole mouse-release path for remote, SyncPlay and queue-advance
playback.**

It is true only of a play begun *inside the browser*. The window handoff has
exactly two unshared paths: `load_feedback.clear()` → `_hand_off`, which
early-returns when `starting is None` and is set only by `_start`'s video
branch (fired only from `ItemActions`); and `on_playstate`'s
`if self._browsing: self._yield()`, which is the **only** path for a remote
"Play To", a SyncPlay start or an EOF queue advance. Verified still present and
still load-bearing at `app.py:2578-2579`.

The reasoning that produced this instruction — "`_start` cleared `_browsing`
first" — is correct about the one caller it looked at and false for every
other. See `docs/POSTMORTEM_3.0.0.md` §2 C3.
