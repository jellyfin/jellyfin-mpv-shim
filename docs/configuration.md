...
- `force_scroll_snapping` - Always quantize scrolling to rows, instead of only when the browser measures itself falling behind. Default: `false`
  - Drawing a new scroll position re-lays the whole overlay and re-issues every visible image, which gets expensive on a very large display and costs more again with an external MPV. So the browser times its own frames, and quantizes to row boundaries **only when a scroll is asking for them faster than it can draw them** — which is when it would otherwise stutter. Anything that keeps up scrolls freely, however fast you spin the wheel. Turn this on if scrolling still stutters for you; the F12 diagnostics overlay shows what a frame costs (`draw:`) if you want to see why. An external MPV is the likeliest case — an image reaches it as a file it has to open and map rather than memory it already shares, so every frame of a scroll costs more there. That cost is inside what the browser measures, so it should quantize on its own; this is the override if it does not. Unrelated to `snapped_scrolling`, which changes what a *notch* does; this changes only what is drawn between notches.
